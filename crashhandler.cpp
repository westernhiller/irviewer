...
DxCrashHandler::DxCrashHandler()
{
	...
	info.pszAppName = L"Irviewer"; // Define application name.
//	info.pszAppVersion = L"2.0.2.3"; // Define application version.
	...
	if (nResult != 0)
	{
		...
		printf_s("%s\n", szErrorMsg);
		QMessageBox::warning(NULL, "irviewer", "Prepare error report handler failed!");
	}
	//      Set crash callback function
	crSetCrashCallback(CrashCallback, NULL);
	// Add the last application log 
	crAddFile2(
        (LPCTSTR)(AddAppPath("irviewer.log")).toLocal8Bit().data(),
        L"irviewer.log",
        L"irviewer file",
