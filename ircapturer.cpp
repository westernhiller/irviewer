...
IrCapturer::IrCapturer(QObject* parent)
    : m_nBytes2Read(0)
    , m_bConnected(false)
    , m_params(nullptr)
    , m_pSettings(nullptr)
...
void IrCapturer::readFromSocket()
{
    m_nHeartbeating = 0;
...
void IrCapturer::parseParams()
{
    ...
    emit updateIntegral((m_params->integration.data32 * 0.34 + 16.32)/ 1000);
    emit updateCycle(m_params->framecycle.data32 * 0.34 / 1000);
}


void IrCapturer::fpareCall(quint8 code)
{
    QByteArray ba;
    ba.resize(11);
    ba[0] = 'f';
...
void IrCapturer::enableHighgain(bool bEnable)
{
    if(bEnable)
...
void IrCapturer::enableNonuniformityCorrection(bool bEnable)
{
    if(bEnable)
...
void IrCapturer::enableIntegralAdjustion(bool bEnable)
{
    if(bEnable)
...
void IrCapturer::setIntegral(double integral)
{
    UNION_REG regvalue;
    regvalue.data32 = static_cast<uint32_t>((integral * 1000 - 16.32) / 0.34);
...
void IrCapturer::setCycle(double cycle)
{
    UNION_REG regvalue;
    regvalue.data32 = static_cast<uint32_t>(cycle  *1000 / 0.34);;
